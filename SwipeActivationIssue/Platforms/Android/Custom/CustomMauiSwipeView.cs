...
{
    public class CustomMauiSwipeView : ContentViewGroup
    {
        const float OpenSwipeThresholdPercentage = 0.6f; // 60%
        const long SwipeAnimationDuration = 200;

        readonly Dictionary<Microsoft.Maui.ISwipeItem, object> _swipeItems;
        ...
        public CustomMauiSwipeView(Context context) : base(context)
        {
            ...
            _swipeItems = new Dictionary<Microsoft.Maui.ISwipeItem, object>();
        ...
        internal void UpdateIsVisibleSwipeItem(Microsoft.Maui.ISwipeItem item)
        {
            if (!_isOpen)
                return;
        ...
        void UpdateSwipeItemViewLayout(ISwipeItemView swipeItemView)
        {
            ...
            Platform.Helper.ViewHandlerExtensions.LayoutVirtualView(handler, 0, 0, (int)swipeItemSize.Width, (int)swipeItemSize.Height);

            Platform.Helper.ElementHandlerExtensions.ToPlatform(swipeItemView?.Handler).InvalidateMeasure(swipeItemView);
        }
        ...
        bool GetIsVisible(Microsoft.Maui.ISwipeItem swipeItem)
        {
        ...
        Size GetSwipeItemSize(Microsoft.Maui.ISwipeItem swipeItem)
        {
            ...
            if (isHorizontal)
            {
                ...
                if (swipeItem is Microsoft.Maui.ISwipeItem)
                {
        ...
        void ExecuteSwipeItem(Microsoft.Maui.ISwipeItem item)
        {
            if (item == null)
                return;
